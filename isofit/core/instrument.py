    ...
    def __init__(self, full_config: Config):
        ...
        for name, idx in self.state_idx.items():
            if name == "PER_WL_SHIFT":
                k = PerWlShift.Sa(self.prior_sigma[idx], self.wl_init)
            elif name == "PER_WL_RCC":
        ...
        if config.per_wl_prior_file is not None:
            (_bounds, _scale, _init, _prior_mean, _prior_cov) = self.load_prior_file(
                config.per_wl_prior_file
            )
            idx = self.state_idx["PER_WL_SHIFT"]
            assert len(_prior_mean) == len(idx)
